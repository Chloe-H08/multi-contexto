...
Contexto ranks guesses by semantic similarity using a small semantic sentence embedding model. This version uses one common noun/object bank: every valid guess is a noun in that bank, every answer comes from that same bank, and every rank is fixed against the full bank.

Current packed bank size: about 6,500 common noun/object entries.

Play it here: https://multi-contexto.vercel.app

## How Ranking Works

- `data/embeddings/manifest.js` and `data/embeddings/chunk-*.js` contain the valid common noun/object list and packed embeddings.
